...
$(TARGET)/stripped-kernel.bin: $(TARGET)/kernel.bin
	strip -sx -o $@ $<

$(TARGET)/disk.img: $(TARGET)/stripped-kernel.bin bootloader/$(TARGET)/bootloader.bin
	@rm -f $@
	bximage -q -hd -mode=flat -size=4 $@
	parted -s $@ -- mklabel msdos
	parted -s -a minimal $@ -- mkpart primary ext2 \
			1s  $(shell dc -e "$(shell stat -c%s $(TARGET)/stripped-kernel.bin) 4095+4096/8*p")s
	parted -s -a minimal $@ -- mkpart primary fat16 \
			$(shell dc -e "$(shell stat -c%s $(TARGET)/stripped-kernel.bin) 4095+4096/8*1+p")s 100%
	parted -s $@ -- set 1 boot on
	dd conv=notrunc if=bootloader/$(TARGET)/bootloader.bin of=$@ bs=446 count=1
	dd conv=notrunc if=$(TARGET)/stripped-kernel.bin of=$@ bs=512 seek=1
